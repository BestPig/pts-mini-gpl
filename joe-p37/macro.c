...
int exsimple(MACRO *m, int arg, int u)
{
	...
	if (ifflag || (cmd->flag&EMETA)) {
		if (flg && u)
			umclear();
		/* Repeat... */
		while ((simple_arg = arg--) && !leave && !ret) {
			ret = execmd(cmd, m->k);
			if (ret == STOP_REPEATING) { ret = 0; break; }
		}
		if (leave)
			return ret;
		if (flg && u)
...
static int uarg_to_arg(int *notify) {
	if (unaarg)
		arg = unaarg;
	else if (negarg)
		arg = 1;
...
static int douargesc1(BW *bw, int c, void *object, int *notify);

static int douargesc0(BW *bw, int c, void *object, int *notify)
{
	if (c != '\033') {
		nungetc(c);
		return uarg_to_arg(notify);
	}
	joe_snprintf_2(msgbuf, JOE_MSGBUFSIZE, joe_gettext(_("URepeat %s%d")), negarg ? "-" : "", unaarg);
	return mkqwna(bw->parent, sz(msgbuf), douargesc1, NULL, NULL, notify) ? 0 : -1;
}
...
static int douargesc1(BW *bw, int c, void *object, int *notify)
{
	...
	joe_snprintf_2(msgbuf, JOE_MSGBUFSIZE, joe_gettext(_("URepeat %s%d")), negarg ? "-" : "", unaarg);
	return mkqwna(bw->parent, sz(msgbuf), douargesc0, NULL, NULL, notify) ? 0 : -1;
}
...
static int douarg(BW *bw, int c, void *object, int *notify)
{
	if (c == '-')
		...
		if (notify)
			*notify = 1;
		return -1;
	} else if (c == '\033') { /* escape, as a Meta- or Alt- prefix */
		return mkqwna(bw->parent, sz(msgbuf), douargesc1, NULL, NULL, notify) ? 0 : -1;
	} else {
		nungetc(c);
		return uarg_to_arg(notify);
	...
	return mkqwna(bw->parent, sz(msgbuf), douarg, NULL, NULL, notify) ? 0 : -1;
}
