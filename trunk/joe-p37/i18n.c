...
/* Sorted list of non-overlapping intervals of combining (non-spacing)
 * characters and high (>127) control characters.
 * originally generated by
 *  "uniset +cat=Me +cat=Mn +cat=Cf -00AD +1160-11FF +200B c"
 * modified by hand since then by **** pts ****
 */
static const struct interval combining_and_control[] = {
	...
	{ 0x200B, 0x200F },  /* control characters */
	{ 0x2028, 0x202E },  /* combining and control characters */
	{ 0x2060, 0x2063 },  /* control characters */
	{ 0x206A, 0x206F },  /* control characters */
	{ 0x20D0, 0x20EA }, { 0x302A, 0x302F }, { 0x3099, 0x309A },
	{ 0xFB1E, 0xFB1E },
	{ 0xFDD0, 0xFDEF },  /* control characters */
	{ 0xFE00, 0xFE0F }, { 0xFE20, 0xFE23 },
	{ 0xFEFF, 0xFEFF },  /* control characters */
	{ 0xFFF9, 0xFFFB },  /* control characters */
	{ 0xFFFE, 0xFFFF },  /* control characters */
	{ 0x1D167, 0x1D169 },
...
int joe_wcwidth(int wide,int ucs)
{
	...
	/* ASCII control characters are one column wide in JOE */
	if (ucs < 32 || ucs == 0x7F)
		return 1;

	if (-1 != bisearch(ucs, combining_and_control, sizeof(combining_and_control) / sizeof(struct interval) - 1)) {
		return ucs <= 0xff ? 4
		     : ucs <= 0xfff ? 5
		     : ucs <= 0xffff ? 6  /* strlen("<FDD0>") == 6 */
		     : ucs <= 0xfffff ? 7
		     : 8;
	}

	if (ucs < double_width[0].first)  /* Shortcut. */
		return 1;
	return 1 + (-1 != bisearch(ucs, double_width, sizeof(double_width) / sizeof(struct interval) - 1));
}
...
int unictrl(int ucs)
{
	...
	return -1 != bisearch(ucs, combining_and_control, sizeof(combining_and_control) / sizeof(struct interval) - 1);
}
