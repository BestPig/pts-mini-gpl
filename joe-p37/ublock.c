...
struct marksav {
	LINK(MARKSAV) link;
	P *markb, *markk;
} markstack = { { &markstack, &markstack}, 0, 0 };
MARKSAV markfree = { {&markfree, &markfree}, 0, 0 };
int nstack = 0;
...
/* Insert the output of `pts-xclip -o' at bw->cursor, and mark it.
 * 
 * Returns 0 on success, -1 or -2 on error.
 */
static int copy_from_pts_xclip(BW *bw) {
	B *tmp;
	...
	is_eof = bw->b->eof->byte == bw->cursor->byte;
	if (!is_eof) pfwrd(markk, 1); /* let markk advance in the inserts below */
	if (0 != load_and_insert(bw, (unsigned char*)"!pts-xclip -o")) {
		umarkk(bw);  /* Make the mark invalid. */
		return 0;
	}

	if (is_eof) pset(markk, bw->b->eof);
	else pbkwd(markk, 1);
	/* umarkb and umarkk have already done updall(), we don't have to. */
        if (markk->byte <= markb->byte) {
		msgnw(bw->parent, joe_gettext(_("No X11 block")));
		return -2;
        }
	return 0;
}
...
int ublkcpy(BW *bw)
{
	...
	} else if (0 != (got = copy_from_pts_xclip(bw))) {
		if (got != -2) msgnw(bw->parent, joe_gettext(_("No block")));
		return -1;
	}
	return 0;
}
...
#endif
		signrm();
		if (fw[0] != 0 && 0 != dup2(fw[0], 0)) abort();
		if (fr[1] != 1 && 1 != dup2(fr[1], 1)) abort();
		if (fr[1] != 2 && 2 != dup2(fr[1], 2)) abort();
		if (fw[0] != 0) close(fw[0]);
		if (fr[1] != 1 && fr[1] != 2) close(fr[1]);
		close(fw[1]);
		close(fr[0]);
#ifdef HAVE_PUTENV
