...
unsigned char defentry_ansilike[] =
    ":co#80:li#25:am"
...
unsigned char defentry_xterm[] =
    ":k0=\\E[21~:k1=\\E[11~:k2=\\E[12~:k3=\\E[13~:k4=\\E[14~"
    ":k5=\\E[15~:k6=\\E[17~:k7=\\E[18~:k8=\\E[19~:k9=\\E[20~:kb=^H"
    ":ke=\\E>:kh=\\E[8~:ks=\\E=:co#80:li#24:cs=\\E[%i%d;%dr"
    ":xn:LP:mi:am:ms:xo:ei=\\E[4l:im=\\E[4h:sr=\\EM"
    ":AL=\\E[%dL:DC=\\E[%dP:DL=\\E[%dM:al=\\E[L:dc=\\E[P:dl=\\E[M"
    ":DO=\\E[%dB:LE=\\E[%dD:RI=\\E[%dC:UP=\\E[%dA:bl=^G:cd=\\E[J"
    ":ce=\\E[K:cl=\\E[H\\E[J:cm=\\E[%i%d;%dH:cr=^M:do=^J:ho=\\E[H"
    ":kd=\\E[B:kl=\\E[D:kr=\\E[C:ku=\\E[A:le=^H:mb=\\E[5m:md=\\E[1m"
    ":me=\\E[m:mh=\\E[2m:nd=\\E[C:se=\\E[m:sf=^J:so=\\E[7m:ta=^I"
    ":ue=\\E[m:up=\\E[A:us=\\E[4m"
    ":";

unsigned char defentry_linux[] =
    ":k1=\\E[[A:k2=\\E[[B:k3=\\E[[C:k4=\\E[[D:k5=\\E[[E:k6=\\E[17~"
    ":k7=\\E[18~:k8=\\E[19~:k9=\\E[20~:kD=\\E[3~:kH=\\E[4~:kI=\\E[2~"
    ":kN=\\E[6~:kP=\\E[5~:kh=\\E[1~:IC=\\E[%d@:ic=\\E[@"
    ":xn:LP:mi:am:ms:xo:ei=\\E[4l:im=\\E[4h:sr=\\EM"
    ":AL=\\E[%dL:DC=\\E[%dP:DL=\\E[%dM:al=\\E[L:dc=\\E[P:dl=\\E[M"
    ":DO=\\E[%dB:LE=\\E[%dD:RI=\\E[%dC:UP=\\E[%dA:bl=^G:cd=\\E[J"
    ":ce=\\E[K:cl=\\E[H\\E[J:cm=\\E[%i%d;%dH:cr=^M:do=^J:ho=\\E[H"
...
CAP *my_getcap(unsigned char *name, unsigned int baud, void (*out) (unsigned char *, unsigned char), void *outptr)
{
	...
	FILE *f, *f1;
	int x, y, c, z, ti;
	unsigned char *tp, *pp, *qq, **npbuf, *idxname;
	...
	if (tp && tp[0] != '/') { /* termcap data in env. e.g. screen(1) */
		cap->tbuf = vsncpy(sv(cap->tbuf), sz(tp));
	}

	npbuf = vamk(0);

	tp = read_user_configs ? (unsigned char *)getenv("HOME") : NULL;
	if (tp) {
		tp = vsncpy(NULL, 0, sz(tp));
		npbuf = vaadd(npbuf, vsncpy(sv(tp), sc("/.joe-p37/termcap")));
	}

	tp = read_user_configs ? (unsigned char *)getenv("HOME") : NULL;
	if (tp) {
		tp = vsncpy(NULL, 0, sz(tp));
		npbuf = vaadd(npbuf, vsncpy(sv(tp), sc("/.termcap")));
	}

	tp = read_user_configs ? (unsigned char *)getenv("TERMCAP") : NULL;
	if (tp && tp[0] == '/') {
		npbuf = vaadd(npbuf, vsncpy(NULL, 0, sz(tp)));
	}

	tp = read_user_configs ? (unsigned char *)getenv("TERMPATH") : NULL;
	if (tp) {
		unsigned char** termbuf = vawords(NULL, sz(tp), sc("\t :"));
		for (y = 0; termbuf[y]; ++y) {
			npbuf = vaadd(npbuf, vsdup(termbuf[y]));
		}
		varm(termbuf);
	}

	if (read_sys_configs) {
		tp = vsncpy(NULL, 0, sc(JOERC));
		npbuf = vaadd(npbuf, vsncpy(sv(tp), sc("termcap")));
		npbuf = vaadd(npbuf, vsncpy(NULL, 0, sc("/etc/termcap")));
	}
...
/*
 ...
 return 0;
*/
		if (0 == zncmp(name, USTR "xterm", 5) ||
		    0 == zncmp(name, USTR "rxvt", 4)) {
			fprintf(stderr, (char *)joe_gettext(_("Couldn't load termcap entry.  Using xterm default\n")));
			defentry = defentry_xterm;
		} else if (0 == zncmp(name, USTR "linux", 5)) {
			fprintf(stderr, (char *)joe_gettext(_("Couldn't load termcap entry.  Using linux default\n")));
			defentry = defentry_linux;
		} else {
			fprintf(stderr, (char *)joe_gettext(_("Couldn't load termcap entry.  Using ANSI-like default\n")));
			defentry = defentry_ansilike;
		}
		f1 = NULL;
		ti = 0;
		cap->tbuf = vsncpy(cap->tbuf, 0, sz(defentry));
		goto checktc;
	}
	...
	if (!f1)
		goto nextfile;
	idxname = vsncpy(NULL, 0, sv(npbuf[y - 1]));
	idxname = vsncpy(sv(idxname), sc(".idx"));
	...
	if (cap->tbuf[x + 1] == 't' && cap->tbuf[x + 2] == 'c' && cap->tbuf[x + 3] == '=') {
		/* Process tc=... entry tail inclusion. */
		name = vsncpy(NULL, 0, sz(cap->tbuf + x + 4));
		cap->tbuf[x] = 0;
		cap->tbuf[x + 1] = 0;
		ti = x + 1;
		sLen(cap->tbuf) = x + 1;
		if (f1 && !f && !fseek(f1, 0, 0)) goto same_file_again;
