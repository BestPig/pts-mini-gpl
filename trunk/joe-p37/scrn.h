...
struct scrn {
	...
	int	co;		/* Screen width */
	int	gtli;		/* Screen height, as reported by ttgtsz, or 0 */
	...
	/* See http://www.delorie.com/gnu/docs/bash/termcap_27.html for the
	 * definitions of am, xn and LP. That page also documents LP, but
	 * that setting is not in Linux terminfo or in termcap read by joe.
	 */
	int	am;		/* Terminal has autowrap, but not magicwrap */
	int	xn;		/* Terminal has magicwrap */
	int	LP;		/* Terminal has DEC-style magicwrap with no scrolling at the last line */
 ...
 *
 * Argument is_from_ttgtsz indicates that the (wid, hei) dimensions are from
 * ttgtsz.
 *
 * Please note that the actual new screen size might be different from (wid,
 * hei), for example there are minimum dimensions and getenv("COLUMNS") and
 * getenv("LINES") override the wid and height arguments.
 *
...
extern int bg_text;
extern int env_lines;  /* from getenv("LINES") */
extern int env_columns;  /* from getenv("COLUMNS") */
extern int notite;
extern int usetabs;
