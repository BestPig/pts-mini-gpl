...
show_version (void)
{
  fprintf (stderr, "wnckkeys %s; xbindkeys by Philippe Brochard\n", WNCKKEYS_VERSION);
}
      ...
      if (pos1)
	{
	  ...
	  if (!pos2 || pos2 > pos1)
	      ...
	      if (pos2 && pos1 < pos2)
		{
		  ...
		  for (p = pos1 + 1, i = 0;
		       p < pos2 && i + 0U < sizeof (command); p++, i++)
		    {
		      command[i] = *p;
		    }
		  command[i] = '\0';
		  ...
		  if (fgets (line, sizeof (line), stream))
		      ...
		      while (*pos1 != '\n')
			{
			  ...
			  if (line2[0] == 'c' && line2[1] == ':')
				...
				{
				  type = CODE;
				  keycode = g_ascii_strtoll (line2+2, (char **) NULL, 0);
				}
			      else
				{
				  keysym = 0;
			  ...
			  if (line2[0] == 'm' && line2[1] == ':')
			    {
			      if (isdigit (line2[2]))
				{
				  modifier |= g_ascii_strtoll (line2+2, (char **) NULL, 0);
				}
			      else
				{
				  keysym = 0;
			  ...
			  if (line2[0] == 'b' && line2[1] == ':')
				...
				{
				  type = BUTTON;
				  button = g_ascii_strtoll (line2+2, (char **) NULL, 0);
				}
			      else
				{
				  keysym = 0;
			  ...
			  else
			    {
			      ...
			      if (g_ascii_strcasecmp (line2, "control") == 0)
				modifier |= ControlMask;
			      else if (g_ascii_strcasecmp (line2, "shift") == 0)
				modifier |= ShiftMask;
			      else if (g_ascii_strcasecmp (line2, "mod1") == 0
				       || g_ascii_strcasecmp (line2, "alt") == 0)
				modifier |= Mod1Mask;
			      else if (g_ascii_strcasecmp (line2, "mod2") == 0)
				modifier |= Mod2Mask;
			      else if (g_ascii_strcasecmp (line2, "mod3") == 0)
				modifier |= Mod3Mask;
			      else if (g_ascii_strcasecmp (line2, "mod4") == 0)
				modifier |= Mod4Mask;
			      else if (g_ascii_strcasecmp (line2, "mod5") == 0)
				modifier |= Mod5Mask;
			      else if (g_ascii_strcasecmp (line2, "release") == 0)
				event_type = RELEASE;
			      else
				{
				  type = SYM;
		 ...
		 unsigned int *button, unsigned int *modifier)
{
  ...
  while(SCM_CONSP(key)){ //Iterate through the list (If it is a list)
    ...
    if (g_ascii_strncasecmp (str, "control", len) == 0)
      *modifier |= ControlMask;
    else if (g_ascii_strncasecmp (str, "shift", len) == 0)
      *modifier |= ShiftMask;
    else if (g_ascii_strncasecmp (str, "mod1", len) == 0
             || g_ascii_strncasecmp (str, "alt", len) == 0)
      *modifier |= Mod1Mask;
    else if (g_ascii_strncasecmp (str, "mod2", len) == 0)
      *modifier |= Mod2Mask;
    else if (g_ascii_strncasecmp (str, "mod3", len) == 0)
      *modifier |= Mod3Mask;
    else if (g_ascii_strncasecmp (str, "mod4", len) == 0)
      *modifier |= Mod4Mask;
    else if (g_ascii_strncasecmp (str, "mod5", len) == 0)
      *modifier |= Mod5Mask;
    else if (g_ascii_strncasecmp (str, "release", len) == 0)
      *event_type = RELEASE;
    else if(strlen (str) > 2 && str[0] == 'm' && str[1] == ':'){
      *modifier |= g_ascii_strtoll (str+2, (char **) NULL, 0);
    ...
    g_free(str); //we copied, so we must destroy this
    str=NULL;
  ...
  if(strlen (str) > 2 && str[1] == ':' && isdigit (str[2]))
	...
	case 'b':
	  *type = BUTTON;
	  *button = g_ascii_strtoll (str+2, (char **) NULL, 0);
	  break;
	case 'c':
	  *type = CODE;
	  *keycode = g_ascii_strtoll (str+2, (char **) NULL, 0);
	  break;
  ...
  g_free(str); //these were used by add key and copied.
...
SCM xbindkey_wrapper(SCM key, SCM cmd)
{
  ...
  //Guile strings are not \0 terminated. hence we must copy.
  cmdstr = g_malloc(sizeof(char) * SCM_LENGTH(cmd) + 1);
  ...
  g_free(cmdstr); //we may get rid of them!
...
SCM run_command_wrapper (SCM command)
{
  ...
  cmdstr = g_malloc(sizeof(char) * SCM_LENGTH(command) + 1);
