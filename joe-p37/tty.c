...
/* needed below: TIOCGWINSZ in <sys/ioctl.h> */
	...
	if (!(m->kpid = fork())) {
		...
		if (!(pid = fork())) {
			...
			if ((x = open((char *)name, O_RDWR)) != -1) {	/* Standard input */
				...
				/* Open stdout, stderr */
					if (1 != dup(x)) abort();
					if (2 != dup(x)) abort();	/* Standard output, standard error */
					...
					(void)!write(1,(char *)buf,zlen(buf));
					sleep(1);

				} else {
					unsigned char buf[1024];
					int len;
					if (2 != dup(x)) abort();  /* Standard error */

					for (;;) {
						len = read(0, buf, sizeof(buf));
						if (len > 0) {
							if (1 != write(1, buf, len)) abort();
						} else {
							break;
						}
					}
