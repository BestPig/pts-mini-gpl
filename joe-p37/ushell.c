...
static int dobuild(BW *bw, unsigned char *s, void *object, int *notify)
{
	...
	a = vaadd(a, cmd);
	a = vaadd(a, s);
	return cstart(bw, USTR "/bin/sh", a, NULL, notify, /*build:*/1, 0);
}

B *buildhist = NULL;

static int build_low(BW *bw, int do_del_first)
{
...
int ubuild(BW *bw)
{
	return build_low(bw, /*do_del_first:*/0);
}

int udelbuild(BW *bw)
{
	return build_low(bw, /*do_del_first:*/1);
}

B *grephist = NULL;

static int grep_low(BW *bw, int do_del_first)
{
	bw->b->parseone = parseone_grep;  /* Set parser to grep */
	/* Since we use dobuild here, it will call cstart(build=1), which
	 * will call cdone_parse, which calls parserrb(bw->bw), which parses
	 * the grep output.
	 */
	if (grephist) {
...
int ugrep(BW *bw)
{
	return grep_low(bw, /*do_del_first:*/0);
}

int udelgrep(BW *bw)
{
	return grep_low(bw, /*do_del_first:*/1);
}
