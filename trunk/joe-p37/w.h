...
int unextvw PARAMS((BASE *bw));			/* Goto next visible window */
int uprevvw PARAMS((BASE *bw));			/* Goto previous visible window */
int umwind PARAMS((BW *bw));			/* Go to message window */
int umfit PARAMS((BW *bw));			/* Fit two windows on screen */
int umsplitfit PARAMS((BW *bw));		/* Split current window and fit these two windows on screen */
