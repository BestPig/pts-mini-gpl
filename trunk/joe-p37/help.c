...
void help_display(Screen *t)
{
	...
	for (y = skiptop; y != t->wind; ++y) {
		if (t->t->updtab[y]) {
				...
				if (*str == '\n' || !*str) {
					...
					if (*str == '\\') {
						...
						case 0:	
							--x;
							continue;
						case 'v': /* \vv to display "3.", the first two bytes of VERSION */
							{	char *s = VERSION; /* e.g. "3.7" */
							 	while (*str == 'v') {
							 		str++; --len;
							 		c = (*s != '\0') ? *s++ : ' ';
									outatr(locale_map, t->t, t->t->scrn + x + y * t->w, t->t->attr + x + y * t->w, x, y, c, atr);
									x++;
								}
							}
						}
			...
			eraeol(t->t, x, y, BG_COLOR(bg_help));
