...
/* Display message at the bottom of the window. */
void msgout(W *w)
{
	SCRN *t = w->t->t;
...
/* Go to previous visible window -- or switch current window to previous. */
int uprevvw(BASE *bw)
{
	...
	if (w != w0) {
		t -> curwin = w;
		return 0;
	} else { /* Only one visible window. Switch it. */
		return wprev(t);
	}
}

/* Go to next visible window -- or switch current window to next. */
int unextvw(BASE *bw)
{
	Screen *t = bw->parent->t;
	W *w0 = t->curwin;
	W *w = w0->link.next;
	while (w != w0 && w->y == -1) w = w->link.next;
	if (w != w0) {
		t -> curwin = w;
		return 0;
	} else { /* Only one visible window. Switch it. */
...
/* Go to previous window, and make it visible. */
int uprevw(BASE *bw)
{
	return wprev(bw->parent->t);
}

/* Go to next window, and make it visible. */
int unextw(BASE *bw)
{
...
int umwind(BW *bw)
{
	...
	if (msgw) {  /* Already visible => hide all other windows. */
		/* The window exists on  */
		bw->parent->t->curwin = msgw;
		wshowone(msgw);
		return 0;
	} else {  /* Replace the current window with errbuf and show only it. */
		msgw = bw->parent;
		if (replace_b_in_bw(bw, errbuf, /*do_orphan:*/1, /*do_restore:*/0, /*use_berror:*/0, /*do_macros:*/0))
			return -1;
		wshowone(msgw);
		return 0;
	}
...
/* Fit previous window and current window on screen. */
static int mfit_low(Screen *t) {
	W *w = t->curwin;
	W *p = findtopw(w)->link.prev->main;
	if (p == w) {
		return -1;
	}
	/* Request size */
...
/* Hide windows other than the current, split current window (unevenly), and
 * fit those two on the screen.
 */
int umsplitfit(BW *bw)
{
	wshowone(bw->parent->main);
	usplitw(bw);
	return mfit_low(bw->parent->main->t);
}
